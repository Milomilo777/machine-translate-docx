...
Last updated: 2026-05-14 (master `HEAD`). Same-day landings since the previous bump: distributable Windows .exe via PyInstaller (`packaging/` directory with `mtd.spec` + `mtd_entry.py` + `README.md`); top-level import surgery so a packaged build boots without MariaDB / hazm / Chrome / undetected_chromedriver installed (`mysql.connector` lazy-loaded, `hazm` + `undetected_chromedriver` wrapped in try/except with passthrough fallbacks); `create_webdriver` fast-path NO-OP for the OpenAI `chatgpt+api` engine path; `MTD_FROZEN_ROOT` env hook so the bundled prompts directory and the central `Log json file/` folder resolve next to the .exe at runtime; FA aligner three-bug fix from AJAR-3147 (grey-only shading, anchored speaker regex, punct-cluster + paren-span split guards) + 5 new regression tests; multi-script polish cycle for zh/vi/ar (universal v2 prompt + `polisher.py` gate so the FA character normaliser only fires when dest_lang==fa — was silently breaking every AR run for months); seven-round reflective prompt cycle that landed `polish_PER.txt` HL-13 NO_SEMICOLON, HL-14 NO_BAASHE, and four new COMMON_MT_PATTERNS entries. Tests: 120/120.
