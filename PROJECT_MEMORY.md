...
Last updated: 2026-05-15 (master `HEAD`). Same-day landings since the previous bump: v7 prompts promoted to canonical (Persian + universal) — six rounds of round-trip critique with GPT-5.5 plus selective injection of 21 features from older iterations (v17–v71); universal prompts restructured into STATIC + JOB_CONFIG layout for OpenAI prompt-cache prefix reuse across language pairs (no more `{SOURCE_LANG}` / `{DEST_LANG}` substitution in system prompt; language identity lives in the user-message `<JOB_CONFIG>` envelope); new `_lang_descriptors.py` curated 80+ locale → rich-descriptor table; `OpenAIPolisher` now takes `source_lang` at construction; `runner.py` plumbs `ctx.language.src_lang` through. Persian v7 polish gained two user-requested locks — LS-12 BROADCAST_OPENING_PATTERNS (canonical "خوش آمدید به X" for SMTV cross-episode consistency) and LS-13 FOREIGN_SCRIPT_AUTHENTIC_VOICE (Lao/Chinese/Sanskrit/etc. greetings preserved byte-id). Polish gained SA-14 ONTOLOGICAL_REPAIR (e.g. "اعتکاف یخچال → عقب‌نشینی"), Q16 ROUND_TRIP back-check, and 4 more SA anchors (MODALITY expanded, COREFERENCE, LOGICAL_CONNECTOR, COMPARISON_SCOPE). Iteration trail and proposals retained in `prompts/*_proposal_v1..v7.txt` + `docs/v7-additions-proposal.md`. Tests: 120/120.
