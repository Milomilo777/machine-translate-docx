...
## Python Test Assets
- **`tests/generate_test_docx.py`**: A python utility for creating dynamic sample Microsoft Word docx instances directly inside the `tests/fixtures/` path.
- **`src/check_chrome.py`**: Asserts the local Chrome Web Browser runtime is fully functional prior to running Selenium requests.
